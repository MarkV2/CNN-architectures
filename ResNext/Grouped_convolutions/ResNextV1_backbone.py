def conv2D(filter_num, kernel_size, strides = (1, 1), padding='valid', use_activation = True, cardinality = 32):
    def block(x):
        x = tf.keras.layers.Conv2D(filter_num, kernel_size, strides=strides, padding=padding, groups=cardinality)(x)
...
def ResNetXtV1(input_shape, output_dims, number_of_identity_blocks):
